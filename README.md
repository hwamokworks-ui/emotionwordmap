...
1. **글자 매칭** — 문장에 감정 단어의 원형·명사형·어간이 그대로 포함돼 있는지 본다. 서로 다른 감정 단어가 여러 개 그대로 들어있으면(예: "아쉽다와 기쁘다, 근데 슬프다") 전부 후보로 보여준다.
2. **AI 의미 분류** — 글자로 못 찾으면 OpenRouter(`openai/gpt-4o-mini`)에게 434개 단어 후보를 주고 가까운 단어를 정확히 7개 고르게 한다. 문장에 여러 감정이 섞여 있으면 각각에 해당하는 단어를 골고루 포함하도록 안내한다.
