...
export async function classifyWordWithLLM(
  ...
  // "감정 먼저 감지 → 체크리스트로 강제" 2단계 호출도 시도해봤지만(2026-08-20), 30문장 평가에서
  // 1순위 정확도가 오히려 크게 떨어져(43.3%→26.7%) 되돌렸다 — 아래 한 줄짜리 지시가 더 낫다.
  const system =
    `너는 한국어 감정 단어 사전에서, 사용자가 쓴 문장이 나타내는 감정에 가까운 단어를 가장 가까운 순서로 정확히 ${count}개 고르는 분류기다. ` +
    `반드시 아래 목록에 있는 id만 고르고, 가장 가까운 순서대로 배열에 담아라. ` +
    '문장에 서로 다른 감정이 여러 개 섞여 있으면(예: 아쉬움과 기쁨이 동시에, 또는 슬픔도 함께), 그 감정들 각각에 해당하는 단어를 골고루 포함해라 — 하나의 감정과 비슷한 단어들로만 채우지 마라. ' +
